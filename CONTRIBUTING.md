...
==========================
Keila is happy you're willing to contribute to our projects. We are usually
very lenient with all submitted PRs, but there are still some guidelines you
can follow to make the approval process go more smoothly.

## Use a Personal Fork and not an Organization

Keila will routinely modify your PR, whether it's a quick rebase or to take care
...
Unfortunately, if you use an organization for your PR, it prevents Keila from
...
- A Java 21 or later JDK (packages vary, use Google/DuckDuckGo/etc.).
    ...
    - Keila requires JDK 21 to build, however, makes use of Gradle's
...
Unlike the Keila API and its implementation, modifications to Paper and Minecraft source files
...
optimizations we have. This makes it easier to update Keila during Minecraft updates,
...
Your commit will be converted into a patch that you can then PR into Keila.
...
Remember that these changes will affect everyone who runs Keila, not just you
and your server.
...
- You need to add a comment with a short and identifiable description of the patch:
  `// Keila start - <COMMIT DESCRIPTION>`
...
- Multi-line changes start with `// Keila start - <COMMIT DESCRIPTION>` and end
  with `// Keila end - <COMMIT DESCRIPTION>`.
- One-line changes should have `// Keila - <COMMIT DESCRIPTION>` at the end of the line.
...
Here's an example of how to mark changes by Keila:

```java
entity.getWorld().dontBeStupid(); // Keila - Move away from beStupid()
entity.getFriends().forEach(Entity::explode);
entity.updateFriends();

// Keila start - Use plugin-set spawn
// entity.getWorld().explode(entity.getWorld().getSpawn());
Location spawnLocation = ((CraftWorld) entity.getWorld()).getSpawnLocation();
entity.getWorld().explode(new BlockPosition(spawnLocation.getX(), spawnLocation.getY(), spawnLocation.getZ()));
// Keila end - Use plugin-set spawn
```
...
public class SomeVanillaClass {
    public final org.bukkit.Location newLocation; // Keila - add location
}
```
...
but the visibility is too low (e.g. a private field in an entity class). Keila can use access transformers
...
When submitting feature patches to Keila, we may ask you to add notes to the patch
...
to make future updates easier on ourselves, Keila tries to use obfuscation
...
```java
double d0 = entity.getX(); final double fromX = d0; // Keila - OBFHELPER
// ...   
this.someMethod(fromX); // Keila
```
...
If this is not done, a developer for Keila might fix that for you before
...
### Using the Keila Test Plugin

The Keila project has a `test-plugin` module for easily testing out API changes
...
of the other Keila run tasks).

### Publishing to Maven local (use in external plugins)

To build and install the Keila APIs and Server to your local Maven repository, do the following:
...
- Add `mavenLocal()` as a repository. Gradle checks repositories in the order they are declared,
  so if you also have the Keila repository added, put the local repository above Keila's.
...
- If you later need to use the Keila-API, you might want to remove the jar
