...
# Usage:
# upstreamCommit --paper HASH --purpur HASH --keila HASH
...
if [ -n "$paperHash" ]; then
    ...
    paper=$(getCommits "PaperMC/Paper" "$paperHash" $(echo $newHash | grep . -q && echo $newHash || echo "dev/26.1")) # Update this on every version update
